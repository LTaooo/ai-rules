...
将仓库克隆到新用户的 `~/ai-rule`，配置各工具的规则入口，并将第三方技能安装在 `~/.agents/skills/`。通过全局规则直接检查两个来源，不需要建立技能目录联接。

## Git 维护

在仓库内检查 `git status` 和差异，只提交全局规则、自定义技能及必要说明。新建自定义技能直接维护 `skills/<技能名>/SKILL.md`，不要把第三方安装目录复制进仓库。
